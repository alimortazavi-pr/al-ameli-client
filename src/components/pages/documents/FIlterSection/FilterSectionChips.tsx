...
export const FilterSectionChips = () => {
  ...
  function selectCategory(category: string) {
    if (query.get("category") === category) {
      router.replace(`${PATHS.DOCUMENTS}${query.delete("category")}`);
    } else {
      router.replace(`${PATHS.DOCUMENTS}${query.add("category", category)}`);
    }
  }
  ...
  function selectTags(tag: string) {
    ...
    router.replace(
      `${PATHS.DOCUMENTS}${query.add("tags", newTagsQuery.join(","))}`
    );
  }
